...
xinit(int cols, int rows)
{
	...
	if (XParseColor(xw.dpy, xw.cmap, colorname[mousebg], &xmousebgi) == 0) {
		xmousebgi.red   = 0x0000;
		xmousebgi.green = 0x0000;
		xmousebgi.blue  = 0x0000;
	}

	XRecolorCursor(xw.dpy, xw.vpointer, &xmousefg, &xmousebgi);
	blankpm = XCreateBitmapFromData(xw.dpy, xw.win, &(char){0}, 1, 1);
	xw.bpointer = XCreatePixmapCursor(xw.dpy, blankpm, blankpm,
					  &xmousefg, &xmousebgi, 0, 0);
